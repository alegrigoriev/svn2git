...
class project_config:
	...
	def add_path_merge_node(self, path_merge_node):
		...
		if node is None:
			raise Exception_cfg_parse("Missing <Rev> node in <MergePath>")
		...
		self.add_revision_action(rev,
				svn_revision_action(b'merge', path, kind=b'dir', copyfrom_path=from_path, copyfrom_rev=from_rev))
		return
	...
	def process_add_file(self, node):
		...
		if rev is None:
			raise Exception_cfg_parse('<AddFile requires Rev="<rev>" attribute')
	...
	def process_formatting_node(self, node):
		...
		try:
			if fmt.style:
				...
				fmt.trim_trailing_backslash = bool_property_value(node, "TrimBackslash", False)
